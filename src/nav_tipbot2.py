...
while True:
    ...
    for item in my_inbox.unread():
        if item.subject.lower() == '!createaccount':
            ...
            withdraw_amount = int(a.GetUserBalance(item.author))
            if int(withdraw_amount) >= 1:
                a.SubtractNavsFromBalance(item.author, int(withdraw_amount))
                txid = withdraw_nav(withdraw_account, int(withdraw_amount))
            ...
            if a.GetUser(item.author) is None:
                ...
                a.DepositRequest(navAddress, comment.author, int(deposit_amount),0, False)
                item.mark_read()
        else:
            try:
                print(item.id)
                ...
                amount = re.search("!tip\s(\d*)\snav /u/NavTipBot", item.body, re.IGNORECASE)
                if amount is not None:
                    ...
                    if a.GetUser(item.author) is None:
                        ...
                        reddit.redditor(str(item.author)).message('Deposit', str(message))
                        a.DepositRequest(navAddress, item.author, int(amount.group(1)), 0, False)
                    elif a.GetUserBalance(item.author) < int(amount.group(1)):
                        navAddress = get_new_address()
                        a.SaveNavAddress(navAddress, item.author)
                        # Reddit message to user asking for a deposit to Nav Address, amount is just a place holder value for now
                        # Doesn't seem to be a way to get the messageID from sent messages in Reddit that I can see. This seems wrong but will investigate
                        ...
                        if a.GetUserBalance(item.author) >= int(amount.group(1)):
                            a.SubtractNavsFromBalance(item.author, int(amount.group(1)))
                            a.AddNavsToBalance(item.submission.author, int(amount.group(1)))
                        ...
                        if a.GetUserBalance(item.author) >= int(amount.group(1)):
                            ...
                            comment.reply("Your Navcoin tip was sent to " + item.submission.author)
                            # Reddit message to user informing them of Nav sent.
                            message = "You have been sent " + amount.group(
                                1) + " Nav coins. To withdraw, reply with !withdraw followed by your Navcoin Wallet Address"
                            reddit.redditor(str(item.submission.author)).message("You've got Nav!", str(message))
                    print(item.id)
                try:
    ...
    for item in to_be_confirmed:
        ...
        if int(confirmed_deposit_amount) > 0:
            a.AddNavsToBalance(reddit_name, int(confirmed_deposit_amount))
            a.ConfirmDeposit(address, int(confirmed_deposit_amount))
